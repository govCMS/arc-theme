 ...
 *
 * Replaces stylesheets with color.module's recolored versions, like
 * _color_html_alter() but for stylesheets with any media query (not just
 * "all").  And for stylesheets added using Omega's non-standard method in
 * .info files; css[global.css].
 *
 * @see _color_html_alter()
 */
function alpha_alpha_process_html(&$vars) {
  global $theme_key;
  ...
  if (module_exists('color')) {
    ...
    if (!empty($color_paths)) {
      ...
      // Replace theme stylesheets if there is a recolored file.
      foreach ($themes[$theme_key]->stylesheets as $paths) {
        foreach ($paths as $base_filename => $old_path) {

          // This part is from _color_html_alter().
        ...
        // Replace the theme's omega-stylesheets if there is a recolored file.
        foreach (array_keys($themes[$theme_key]->info['css']) as $filename) {

          // Loop over the paths of the recolored stylesheet files.
          foreach ($color_paths as $color_path) {

            // Find recolored stylesheets with the same name as the
            // omega-stylesheets.
            if ($filename == drupal_basename($color_path)) {

              // Index all CSS files without their omega-layout prefixes.
              if (!isset($index)) {
                $index = array();
                foreach (array_keys($vars['css']) as $name) {
                  $key = array_pop(explode('::', $name));
                  $index[$key][] = $name;
                }
              }

              // Omega-stylesheets are always found in the theme's css
              // directory.
              $path = drupal_get_path('theme', $theme_key) . "/css/$filename";

              // Replace all references to the omega-stylesheet with the
              // recolored stylesheet.
              if (!empty($index[$path])) {
                foreach ($index[$path] as $css_path) {
                  $vars['css'][$css_path]['data'] = $color_path;
                }
              }
            }
          }
