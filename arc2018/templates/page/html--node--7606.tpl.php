            ...
            <tbody id="outcomes-table-body">
              ...
              <tr>
                ...
                {{#each outcomes.era2012}}
                    <td data-year="2012" data-rating="{{rating}}" aria-label="ERA 2012 rating: {{rating}}">{{rating}}</td>
                {{/each}}
              </tr>
              {{/each}}
              {{/each}}
...
jQuery( document ).ready(function() {
    ...
    function getInstitutionNames() {
        return $.getJSON('<?php print $path_to_arc; ?>/resources/data/2012_Institutions.json').then( function (institutionNames, status, jqXHR) {
            //todo handle failure
            return institutionNames;
        },
        function (data, status, jqXHR) {
            console.log("get 2012_Institutions.json failed");
        });
    }
    ...
    function getFieldOfResearchNames() {
        return $.getJSON('<?php print $path_to_arc; ?>/resources/data/2015_ForCodes.json').then( function (fieldOfResearchNames, status, jqXHR) {
         ...
         function (data, status, jqXHR) {
             console.log("get 2012_ForCodes.json failed");
         });
    }
