...
/**
 * @file
 * Template preprocessor functions for the arc theme.
 */

/**
 * Implements hook_preprocess_html().
 */
function arc_preprocess_html(&$vars) {
  ...
  if ($colour_toggle == '1') {
    ...
    drupal_add_css(drupal_get_path('theme', 'arc') . '/css/colour_schemes/' . $colour_scheme . '/theme-colour.css', array(
      'media' => 'screen',
      'group' => CSS_THEME,
    ));
  }
 ...
 */
function arc_process_html(&$vars) {
 ...
 */
function arc_preprocess_node(&$vars) {
 ...
 */
function arc_preprocess_entity(&$vars) {
 ...
 */
function arc_form_alter(&$form, &$form_state, $form_id) {
 ...
 */
function arc_breadcrumb($variables) {
  ...
  if (!empty($breadcrumb)) {
    ...
    $crumbs = '<div class="breadcrumb">'. t('<span class="static_here">You are here:</span> ');
 ...
 */
function arc_preprocess_maintenance_page(&$variables) {
 ...
 */
function arc_views_more($variables) {
...
function sev_preprocess_page(&$variables) {
    if (isset($variables['node']->type)) {
        ...
        $variables['theme_hook_suggestions'][] = 'page--' . $nodetype;
    }
}